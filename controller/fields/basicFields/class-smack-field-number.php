...
class SmackFieldNumber
{
	...
	public function render_number_field($field, $smack_field_name, $smack_field_label, $smack_field_value, $smack_field_instructions, $smack_field_placeholder, $smack_field_prepend, $smack_field_append, $smack_field_min, $smack_field_max, $smack_field_stepsize, $page_type, $source){
		$number_id = "tools-engine-" . $field['field_index'];
		...
		if($source == 'via_group'){
			return $number_field_array;
		}
		
		?><div 
				id="<?php echo esc_attr($number_id) ?>" 
				data-params="<?php echo htmlspecialchars(json_encode($number_field_array), ENT_QUOTES, 'UTF-8'); ?>" >
			</div>
		<?php
	}
}